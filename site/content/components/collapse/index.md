...
layout: docs
title: Collapse
description: "Collapse are expansion panel allows users can expand or collapse the long sections of information."
toc: true
tabs: true
group: surfaces
menu:
...
Responsive collapse component built with the latest Bootstrap 5. Collapse are expansion panel allows users can expand or collapse the long sections of information.

Collapse are toggle the visibility of content across your project with a few classes and our JavaScript plugins.
...
Generally, we recommend using a `<button>` with the `data-bs-target` attribute, the `data-bs-toggle="collapse"` is required. While not recommended from a semantic point of view, you can also use a `<a>` with the `href` attribute (and a `role="button"`).

## Horizontal 

The collapse plugin also supports horizontal collapsing. Add the `.collapse-horizontal` modifier class to transition the width instead of height and set a width on the immediate child element. Feel free to write your own custom Sass, use inline styles, or use our [width utilities]({{< docsref "width" >}}).
